	...
	*/
...

	{
		...
		// 정보 제공을 위한 리스트 선언
			...
			strQuery = strQuery + " WHERE changed_number > " + number;			
			...
			//Get all CCTVs deleted
			List<TrOASISCCTVLog> cctvDelList1 = new ArrayList<TrOASISCCTVLog>();
			...
			while (iterator.hasNext()){
				...
				if (log.getChanged_type() == 1) {
					for (int i=0; i<cctvDelList1.size(); i++) {
						TrOASISCCTVLog delLog = cctvDelList1.get(i);

						if(log.getCctv_id().equals(delLog.getCctv_id()) 
								&& log.getChanged_number() < delLog.getChanged_number()) {
							cctvDelList2.add(delLog.getCctv_id());
						} 
					}					
				}
			}
			...
			//Delete Filtering 1 : Add [Modify] Delete
			...
			//Delete Filtering 2 : Modify Delete
			List<TrOASISCCTVLog> finalLogList2 = new ArrayList<TrOASISCCTVLog>();
			for (int j=0; j<finalLogList1.size();j++){
				TrOASISCCTVLog log = finalLogList1.get(j);
...
<troasis>
	...
	<cctv_changed_list>
<%
	for (int l=0; l<finalLogList2.size();l++){
		TrOASISCCTVLog log = finalLogList2.get(l);
...
<%
		if (log.getChanged_type() != 2){
			...
			for(int i=0; i<finalCCTVList.size();i++){
				if (finalCCTVList.get(i).getCctvId().equals(log.getCctv_id())){
					cctv = finalCCTVList.get(i);
					break;
				}
			}
