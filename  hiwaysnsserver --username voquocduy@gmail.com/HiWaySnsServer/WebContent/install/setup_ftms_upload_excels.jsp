...
<%
	try
	{
		...
		//Node 증가방향의 교통 정보.
		query	= query + ", inc_info					VARCHAR(2048) DEFAULT ''";	//소통정보 - 시간.
		query	= query + ", inc_info_speed				VARCHAR(2048) DEFAULT ''";	//소통정보 - 속력.
		query	= query + ", inc_speed					INT DEFAULT 0";				//평균속력. 단위는 Km/H.

		//Node 감소방향의 교통 정보.
		query	= query + ", dec_info					VARCHAR(2048) DEFAULT ''";	//소통정보 - 시간.
		query	= query + ", dec_info_speed				VARCHAR(2048) DEFAULT ''";	//소통정보 - 속력.
	...
	for ( int key = 0; key < list_file_names.length; key++ )
	{
		...
		try
		{
			...
			if ( list_file_names[key] == "FTMS_node_data.xls" )
				...
				query = "INSERT INTO troasis_ftms_traffic(agent_id, agent_name, road_name, road_no, loc_lng, loc_lat, inc_info, dec_info, inc_speed, dec_speed, inc_info_speed, dec_info_speed) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, '', '')";
