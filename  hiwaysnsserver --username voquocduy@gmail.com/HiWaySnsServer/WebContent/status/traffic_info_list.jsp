...
<head>
	...
	<title>TrOASIS 운영 및 관리 홈페이지 - 사용자 교통정보 등록 현황</title>
</head>
...
<body>
	<a href="traffic_info_list.jsp">새로고침</a>
	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	...
	<table width="1460">
		...
		<tr height="20">
			<td colspan="4" align="left">
				...
				<select class="combo" id="sort_order" name="sort_order" style="width: 160px; color:#000000; background-color:#ffffff;" Onchange="return js_page_refresh();">
					...
					<option value="4" <% if ( sort_order == 4 ) { %>selected<% } %>>Link ID</option>
				</select>
			</td>
		...
		<tr height="10"><td colspan="16"></td></tr>


		<tr height="4"><td colspan="16" background="images/dot.gif"></td></tr>
		<tr><td colspan="16" align="center"><strong>교통정보 현황</strong></td></tr>
		<tr height="4"><td colspan="16" background="images/dot.gif"></td></tr>
		<tr>
...
<%
	try
	{
		...
		//데이터베이스에서 교통정보목록의 개수 읽어오기.
		String	query;
		query	= "SELECT COUNT(*) FROM troasis_user_traffic";
		...
		//데이터베이스에서 교통정보 목록을 읽어와서 출력한다.
		query	= "SELECT * FROM troasis_user_traffic";
		//query	= query + " WHERE flag_deleted = 0 ";
		//query	= query + " WHERE id = log_id";		
		if ( search_text.length() > 0 )
		{
		...
		case 3	:
			...
			break;
		case 4	:
			query	= query + " ORDER BY link_id ASC, id DESC ";
			break;
		default	:
...
%>
		<tr height="1"><td colspan="16" background="images/dot.gif"></td></tr>
		<tr>
...
<%
	...
	{
		//완료 메시지.
		out.println( "<br><br>교통정보 현황을 읽어오는 과정에서 오류가 발생 했습니다." );
...
%>
		...
		<tr height="20"><td colspan="16"></td></tr>
		<tr height="20">
			<td colspan="16" align="center"><%=param.put_page_list(count, page_size, page_no)%></td>
		</tr>
	</table>
	</form>
