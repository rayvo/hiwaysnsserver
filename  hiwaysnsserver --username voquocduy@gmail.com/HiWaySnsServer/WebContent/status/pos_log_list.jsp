...
<head>
	...
	<title>TrOASIS 운영 및 관리 홈페이지 - 사용자의 주행기록 현황</title>
</head>
...
<body>
	<a href="pos_log_list.jsp">새로고침</a>
	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	...
	<table width="900">
		...
		<tr height="20">
			...
			<td colspan="4" align="left">
				...
				<select class="combo" id="search_type" name="search_type" style="width: 160px; color:#000000; background-color:#ffffff;">
					<option value="0" <% if ( search_type < 1) { %>selected<% } %>>전체</option>
		...
		<tr height="4"><td colspan="7" background="images/dot.gif"></td></tr>
		<tr><td colspan="7" align="center"><strong>사용자의 주행기록 현황</strong></td></tr>
		<tr height="4"><td colspan="7" background="images/dot.gif"></td></tr>
		<tr>
			<td width="50" align="center">id</td>
			<td width="100" align="center">사용자 ID</td>
			<td width="200" align="center">로그 시각</td>
			<td width="150" align="center">위도</td>
			<td width="150" align="center">경도</td>
			<td width="150" align="center">속도</td>
			<td width="150" align="center">평균속력</td>
		</tr>
...
<%
	try
	{
		...
		//데이터베이스에서 로그 목록의 개수 읽어오기.
		String	query;
		query	= "SELECT COUNT(*) FROM troasis_pos_log";
		...
		//데이터베이스에서 로그 목록을 읽어와서 출력한다.
		query	= "SELECT * FROM troasis_pos_log";
		//query	= query + " WHERE flag_deleted = 0 ";
		if ( search_text.length() > 0 )
		{
			switch( search_type )
		...
		dTime_time_log	= db.getTimestampString ( db.mDbRs.getLong( "time_log" ) );
			...
			int		db_speed_avg	= db.mDbRs.getInt( "speed_avg" );
%>
		<tr height="1"><td colspan="7" background="images/dot.gif"></td></tr>
		<tr>
			...
			<td align="center"><%=db_speed_avg %>Km/H</td>
		</tr>
<%
		}
	}
	...
	{
		//완료 메시지.
		out.println( "<br><br>사용자의 주행기록 현황을 읽어오는 과정에서 오류가 발생 했습니다." );
...
%>
		...
		<tr height="20">
			<td colspan="7" align="center"><%=param.put_page_list(count, page_size, page_no)%></td>
		</tr>
	</table>
	</form>
